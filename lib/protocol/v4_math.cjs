'use strict'

const Q96 = 2n ** 96n
const Q128 = 2n ** 128n

const MIN_TICK = -887272
const MAX_TICK = 887272

function assertTickRange(tick) {
  if (!Number.isInteger(tick) || tick < MIN_TICK || tick > MAX_TICK) {
    throw new Error(`tick ${tick} is outside supported range`)
  }
}

function getSqrtRatioAtTick(tick) {
  assertTickRange(tick)
  let absTick = BigInt(Math.abs(tick))
  let ratio = (absTick & 0x1n) !== 0n
    ? 0xfffcb933bd6fad37aa2d162d1a594001n
    : 0x100000000000000000000000000000000n

  if ((absTick & 0x2n) !== 0n) ratio = (ratio * 0xfff97272373d413259a46990580e213an) >> 128n
  if ((absTick & 0x4n) !== 0n) ratio = (ratio * 0xfff2e50f5f656932ef12357cf3c7fdccn) >> 128n
  if ((absTick & 0x8n) !== 0n) ratio = (ratio * 0xffe5caca7e10e4e61c3624eaa0941cd0n) >> 128n
  if ((absTick & 0x10n) !== 0n) ratio = (ratio * 0xffcb9843d60f6159c9db58835c926644n) >> 128n
  if ((absTick & 0x20n) !== 0n) ratio = (ratio * 0xff973b41fa98c081472e6896dfb254c0n) >> 128n
  if ((absTick & 0x40n) !== 0n) ratio = (ratio * 0xff2ea16466c96a3843ec78b326b52861n) >> 128n
  if ((absTick & 0x80n) !== 0n) ratio = (ratio * 0xfe5dee046a99a2a811c461f1969c3053n) >> 128n
  if ((absTick & 0x100n) !== 0n) ratio = (ratio * 0xfcbe86c7900a88aedcffc83b479aa3a4n) >> 128n
  if ((absTick & 0x200n) !== 0n) ratio = (ratio * 0xf987a7253ac413176f2b074cf7815e54n) >> 128n
  if ((absTick & 0x400n) !== 0n) ratio = (ratio * 0xf3392b0822b70005940c7a398e4b70f3n) >> 128n
  if ((absTick & 0x800n) !== 0n) ratio = (ratio * 0xe7159475a2c29b7443b29c7fa6e889d9n) >> 128n
  if ((absTick & 0x1000n) !== 0n) ratio = (ratio * 0xd097f3bdfd2022b8845ad8f792aa5825n) >> 128n
  if ((absTick & 0x2000n) !== 0n) ratio = (ratio * 0xa9f746462d870fdf8a65dc1f90e061e5n) >> 128n
  if ((absTick & 0x4000n) !== 0n) ratio = (ratio * 0x70d869a156d2a1b890bb3df62baf32f7n) >> 128n
  if ((absTick & 0x8000n) !== 0n) ratio = (ratio * 0x31be135f97d08fd981231505542fcfa6n) >> 128n
  if ((absTick & 0x10000n) !== 0n) ratio = (ratio * 0x09aa508b5b7a84e1c677de54f3e99bc9n) >> 128n
  if ((absTick & 0x20000n) !== 0n) ratio = (ratio * 0x05d6af8dedb81196699c329225ee604n) >> 128n
  if ((absTick & 0x40000n) !== 0n) ratio = (ratio * 0x02216e584f5fa1ea926041bedfe98n) >> 128n
  if ((absTick & 0x80000n) !== 0n) ratio = (ratio * 0x48a170391f7dc42444e8fa2n) >> 128n

  if (tick > 0) {
    ratio = ((1n << 256n) - 1n) / ratio
  }

  return (ratio + ((1n << 32n) - 1n)) >> 32n
}

function mulDiv(a, b, denominator) {
  return (BigInt(a) * BigInt(b)) / BigInt(denominator)
}

function getLiquidityForAmount0(sqrtRatioAX96, sqrtRatioBX96, amount0) {
  let sqrtA = BigInt(sqrtRatioAX96)
  let sqrtB = BigInt(sqrtRatioBX96)
  if (sqrtA > sqrtB) {
    const next = sqrtA
    sqrtA = sqrtB
    sqrtB = next
  }
  const intermediate = mulDiv(sqrtA, sqrtB, Q96)
  return mulDiv(BigInt(amount0), intermediate, sqrtB - sqrtA)
}

function getLiquidityForAmount1(sqrtRatioAX96, sqrtRatioBX96, amount1) {
  let sqrtA = BigInt(sqrtRatioAX96)
  let sqrtB = BigInt(sqrtRatioBX96)
  if (sqrtA > sqrtB) {
    const next = sqrtA
    sqrtA = sqrtB
    sqrtB = next
  }
  return mulDiv(BigInt(amount1), Q96, sqrtB - sqrtA)
}

function getLiquidityForAmounts(sqrtRatioX96, sqrtRatioAX96, sqrtRatioBX96, amount0, amount1) {
  let sqrtA = BigInt(sqrtRatioAX96)
  let sqrtB = BigInt(sqrtRatioBX96)
  if (sqrtA > sqrtB) {
    const next = sqrtA
    sqrtA = sqrtB
    sqrtB = next
  }

  const current = BigInt(sqrtRatioX96)
  const normalizedAmount0 = BigInt(amount0 || 0)
  const normalizedAmount1 = BigInt(amount1 || 0)

  if (current <= sqrtA) {
    return getLiquidityForAmount0(sqrtA, sqrtB, normalizedAmount0)
  }
  if (current < sqrtB) {
    const liquidity0 = getLiquidityForAmount0(current, sqrtB, normalizedAmount0)
    const liquidity1 = getLiquidityForAmount1(sqrtA, current, normalizedAmount1)
    return liquidity0 < liquidity1 ? liquidity0 : liquidity1
  }
  return getLiquidityForAmount1(sqrtA, sqrtB, normalizedAmount1)
}

function getAmount0ForLiquidity(sqrtRatioAX96, sqrtRatioBX96, liquidity) {
  let sqrtA = BigInt(sqrtRatioAX96)
  let sqrtB = BigInt(sqrtRatioBX96)
  if (sqrtA > sqrtB) {
    const next = sqrtA
    sqrtA = sqrtB
    sqrtB = next
  }
  return mulDiv(BigInt(liquidity), (sqrtB - sqrtA) * Q96, sqrtB * sqrtA)
}

function getAmount1ForLiquidity(sqrtRatioAX96, sqrtRatioBX96, liquidity) {
  let sqrtA = BigInt(sqrtRatioAX96)
  let sqrtB = BigInt(sqrtRatioBX96)
  if (sqrtA > sqrtB) {
    const next = sqrtA
    sqrtA = sqrtB
    sqrtB = next
  }
  return mulDiv(BigInt(liquidity), sqrtB - sqrtA, Q96)
}

function getAmountsForLiquidity(sqrtRatioX96, sqrtRatioAX96, sqrtRatioBX96, liquidity) {
  let sqrtA = BigInt(sqrtRatioAX96)
  let sqrtB = BigInt(sqrtRatioBX96)
  if (sqrtA > sqrtB) {
    const next = sqrtA
    sqrtA = sqrtB
    sqrtB = next
  }

  const current = BigInt(sqrtRatioX96)
  const normalizedLiquidity = BigInt(liquidity || 0)
  if (normalizedLiquidity <= 0n) {
    return {
      amount0: 0n,
      amount1: 0n
    }
  }

  if (current <= sqrtA) {
    return {
      amount0: getAmount0ForLiquidity(sqrtA, sqrtB, normalizedLiquidity),
      amount1: 0n
    }
  }

  if (current < sqrtB) {
    return {
      amount0: getAmount0ForLiquidity(current, sqrtB, normalizedLiquidity),
      amount1: getAmount1ForLiquidity(sqrtA, current, normalizedLiquidity)
    }
  }

  return {
    amount0: 0n,
    amount1: getAmount1ForLiquidity(sqrtA, sqrtB, normalizedLiquidity)
  }
}

function decodeClTickSpacing(parameters) {
  const raw = (BigInt(parameters || 0) >> 16n) & 0xffffffn
  const signed = raw >= 0x800000n ? raw - 0x1000000n : raw
  return Number(signed)
}

module.exports = {
  Q96,
  Q128,
  MIN_TICK,
  MAX_TICK,
  getSqrtRatioAtTick,
  getAmount0ForLiquidity,
  getAmount1ForLiquidity,
  getAmountsForLiquidity,
  getLiquidityForAmount0,
  getLiquidityForAmount1,
  getLiquidityForAmounts,
  decodeClTickSpacing
}
